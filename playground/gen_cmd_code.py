...
async def gen_cmd(conv_id, cmd, host='localhost:8000'):
    # 10.64.136.2:1003
    url = f"http://{host}/llm/gen_cmd/{conv_id}"
...
async def gen_code(conv_id, host='localhost:8000'):
    # 10.64.136.2:1003
...
async def handle_request(semaphore, session, cmd):
    async with semaphore:
        try:
            ...
            await gen_cmd(conv_id, cmd)
            await gen_code(conv_id)
        except Exception as e:
            print(f'failed with conv_id: {conv_id}. Error: {e}')
            return False

async def main(commands):
    semaphore = asyncio.Semaphore(8)  # Limit to 10 concurrent requests
